...
namespace DatabaseMigrationTool.Views
{
    public partial class MainWindow : Window
    {
        ...
        // Connection manager for centralized connection handling
        ...
        private void BrowseExportOutputDirectory(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                _settingsService.AddRecentDirectory(dialog.SelectedPath, "output");
            }
        }
        ...
        private void BrowseImportInputDirectory(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                _settingsService.AddRecentDirectory(dialog.SelectedPath, "input");
            }
        }
        ...
        private void BrowseSchemaOutputDirectory(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                _settingsService.AddRecentDirectory(dialog.SelectedPath, "script");
            }
        }
        ...
        private void BrowseTableCriteriaFile(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                ...
                _settingsService.AddRecentFile(dialog.FileName, "criteria");
            }
        }
        ...
        private async void OpenCriteriaHelper(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    await Task.Run(async () =>
                    {
                        ...
                        var connection = connectionResult.Data!; // Data is guaranteed to be non-null when Success is true
                        try
                        {
                            // Get all tables or just selected ones
                            var allTables = await provider.GetTablesAsync(connection).ConfigureAwait(false);
                            ...
                            foreach (var table in tables)
                            {
                                try
                                {
                                    var columns = await provider.GetColumnsAsync(connection, table.Name, table.Schema).ConfigureAwait(false);
                    ...
                    // Get existing criteria file path and open the criteria helper window on UI thread
                    Dispatcher.Invoke(() =>
                    {
            ...
            {
                ErrorHandler.HandleError(ex, "Load Table Information for Criteria Helper");
            }
        }
        ...
        private async void ViewSchema(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // Standard handling for database types using ConnectionManager
                var connectionResult = await _connectionManager.GetConnectionAsync(providerName, SchemaConnectionString).ConfigureAwait(false);
                if (!connectionResult.Success)
                {
                    Dispatcher.Invoke(() => StatusTextBlock.Text = $"Error getting connection: {connectionResult.ErrorMessage}");
                    Dispatcher.Invoke(() => MessageBox.Show($"Error getting connection: {connectionResult.ErrorMessage}\\n\\nDetails: {connectionResult.Exception?.Message}", "Connection Error", MessageBoxButton.OK, MessageBoxImage.Error));
                    return;
                }
                ...
                try
                {
                    Dispatcher.Invoke(() => StatusTextBlock.Text = "Connection opened successfully");
                    
                    // Log connection info for diagnostic purposes
                    string connectionInfo = $"Database: {connection.Database ?? "Unknown"}, Provider: {providerName}, State: {connection.State}";
                    Dispatcher.Invoke(() => StatusTextBlock.Text = $"Reading schema... ({connectionInfo})");
                    
                    // Get tables with retry logic
                    tables = await GetTablesWithRetryAsync(provider, connection, tableNames).ConfigureAwait(false);
                    Dispatcher.Invoke(() => StatusTextBlock.Text = $"Schema retrieved: {tables.Count} tables found");
                }
                catch (Exception ex)
                {
                    Dispatcher.Invoke(() => StatusTextBlock.Text = $"Error retrieving schema: {ex.Message}");
                    Dispatcher.Invoke(() => MessageBox.Show($"Error retrieving schema: {ex.Message}\n\nDetails: {ex}", "Schema Error", MessageBoxButton.OK, MessageBoxImage.Error));
                    return;
                }
                finally
                {
                ...
                {
                    Dispatcher.Invoke(() => StatusTextBlock.Text = "No tables found in database");
                    Dispatcher.Invoke(() => MessageBox.Show("No tables were found in the database. Check your connection string and table names if specified.", "No Tables Found", MessageBoxButton.OK, MessageBoxImage.Information));
                    return;
                }

                Dispatcher.Invoke(() => StatusTextBlock.Text = $"Displaying schema for {tables.Count} tables...");
                ...
                bool shouldGenerateScripts = false;
                string scriptPath = "";
                Dispatcher.Invoke(() => {
                    shouldGenerateScripts = (SchemaScriptOutputCheckBox.IsChecked ?? false) && !string.IsNullOrEmpty(SchemaScriptPathTextBox.Text);
                    scriptPath = SchemaScriptPathTextBox.Text;
                });
                
                if (shouldGenerateScripts)
                {
                    Dispatcher.Invoke(() => StatusTextBlock.Text = "Generating SQL scripts...");
                    await GenerateSqlScripts(tables, scriptPath, providerName);
                    Dispatcher.Invoke(() => StatusTextBlock.Text = "SQL scripts generated successfully");
                }

                Dispatcher.Invoke(() => StatusTextBlock.Text = "Schema view completed");
            }
            catch (Exception ex)
            {
                Dispatcher.Invoke(() => StatusTextBlock.Text = $"Error in schema view: {ex.Message}");
                ErrorHandler.HandleError(ex, "Schema View Operation");
            }
            finally
            {
                Dispatcher.Invoke(() => Mouse.OverrideCursor = null);
        ...
        private async Task GenerateSqlScripts(List<TableSchema> tables, string outputPath, string providerName)
        {
            ...
            foreach (var table in tables)
            {
                try
                {
                    ...
                    await File.WriteAllTextAsync(fileName, script).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
        ...
        private void BrowseTables(ConnectionStringControl connectionControl, TextBox tablesTextBox, string operation)
        {
            try
            {
            ...
            {
                ErrorHandler.HandleError(ex, "Browse Tables");
            }
        }
        ...
        private async void BrowseImportTablesFromExportData()
        {
            try
            {
                ...
                try
                {
                    ...
                    await Task.Run(async () =>
                    {
                        var exportMetadata = await MetadataManager.ReadMetadataAsync(importPath).ConfigureAwait(false);
                    ...
                    // Parse existing selected tables and show table selection window on UI thread
                    Dispatcher.Invoke(() =>
                    {
                        List<string>? preselectedTables = null;
                        if (!string.IsNullOrWhiteSpace(ImportTablesTextBox.Text))
            ...
            {
                ErrorHandler.HandleError(ex, "Load Export Data for Import");
            }
        }
        ...
        private void ResumeOperations_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ErrorHandler.HandleError(ex, "Open Recovery Window");
            }
        }
        ...
        private async void ResumeExportOperation(OperationState operation)
        {
            try
            {
                ...
                bool success = await _recoveryService.ResumeExportOperationAsync(
                    operation, provider, operation.ConnectionString, progressReporter).ConfigureAwait(false);
                ...
                if (success)
                {
                    ...
                    // Hide progress UI after successful resume
                    ExportProgressGroupBox.Visibility = Visibility.Collapsed;
                    ExportProgressBar.IsIndeterminate = false;
                    ExportProgressBar.Value = 0;
                }
        ...
        private async void SaveConfiguration_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var saveDialog = new SaveFileDialog
                {
                    ...
                    FileName = ConfigurationManager.GetDefaultConfigFileName("migration")
                };
                ...
                if (saveDialog.ShowDialog() == true)
                {
                    ...
                    var config = ConfigurationManager.CreateFromGuiState(
                    ...
                    await ConfigurationManager.SaveConfigurationAsync(config, saveDialog.FileName);
            ...
            {
                ErrorHandler.HandleError(ex, "Save Configuration");
            }
        }
        ...
        private async void LoadConfiguration_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    var config = await ConfigurationManager.LoadConfigurationAsync(openDialog.FileName);
            ...
            {
                ErrorHandler.HandleError(ex, "Load Configuration");
            }
        }
        
        private ExportConfig GatherExportConfiguration()
        {
        ...
        private ImportConfig GatherImportConfiguration()
        {
        ...
        private SchemaConfig GatherSchemaConfiguration()
        {
        ...
        private void ApplyConfigurationToGui(MigrationConfiguration config)
        {
        ...
        private void SetProviderByName(string providerName, string tabType)
        {
            ...
            int index = Array.FindIndex(providers, p => p.Equals(providerName, StringComparison.OrdinalIgnoreCase));
        ...
        #region Retry Logic Helpers
        
        private async Task<List<TableSchema>> GetTablesWithRetryAsync(IDatabaseProvider provider, System.Data.Common.DbConnection connection, List<string>? tableNames)
        {
            var errorInfo = new ErrorInfo
            {
                Context = "Retrieve Database Schema",
                Severity = ErrorSeverity.Error,
                IsRecoverable = true
            };
            
            // Use ErrorHandler's retry mechanism
            bool success = await ErrorHandler.TryRecoverAsync(errorInfo, async () =>
            {
                try
                {
                    var tables = await provider.GetTablesAsync(connection, tableNames).ConfigureAwait(false);
                    return tables?.Count > 0; // Consider successful if we get any tables
                }
                catch (Exception ex)
                {
            ...
            }, _settingsService.Settings.Performance.MaxRetryAttempts);
            
            if (!success)
            {
                // If retry failed, try one final time and let exception bubble up
                return await provider.GetTablesAsync(connection, tableNames).ConfigureAwait(false);
            }
            
            // If retry succeeded, get the actual data
            return await provider.GetTablesAsync(connection, tableNames).ConfigureAwait(false);
        }
        ...
        private async Task LoadAndApplyUserSettingsAsync()
        {
            try
            {
                ...
                // Ensure ApplyUserSettings runs on the UI thread
                Dispatcher.Invoke(() => ApplyUserSettings());
            }
            catch (Exception ex)
            {
        ...
        private void ApplyUserSettings()
        {
            ...
            if (!double.IsNaN(settings.MainWindow.Left) && !double.IsNaN(settings.MainWindow.Top))
            {
            ...
            ExportBatchSizeTextBox.Text = settings.Defaults.BatchSize.ToString();
            ImportBatchSizeTextBox.Text = settings.Defaults.BatchSize.ToString();
            
            ImportCreateSchemaCheckBox.IsChecked = settings.Defaults.CreateSchema;
            ...
            // Note: TabControl selection would go here if we had access to it
        }
        ...
        private async void SaveUserSettings()
        {
            try
            {
                ...
                // Update default values from UI
                if (int.TryParse(ExportBatchSizeTextBox.Text, out int batchSize))
                {
                    _settingsService.Settings.Defaults.BatchSize = batchSize;
                }
