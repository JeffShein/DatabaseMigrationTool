...
namespace DatabaseMigrationTool.Services
{
    ...
    public class ConnectionManager
    {
        ...
        public ConnectionManager(string connectionString, IDatabaseProvider provider, int maxConnections = 10)
        {
        ...
        public async Task<T> UseConnectionAsync<T>(Func<DbConnection, Task<T>> action)
        {
        ...
        private async Task<DbConnection> GetConnectionAsync()
        {
            ...
            {
                connection = _provider.CreateConnection(_connectionString);
