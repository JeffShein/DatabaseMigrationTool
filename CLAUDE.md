...
- .NET 9.0 SDK (version 9.0.203 specified in global.json)
...
dotnet run --project src/DatabaseMigrationTool -- diagnose --file ./export/batch0.bin
dotnet run --project src/DatabaseMigrationTool -- validate-ei --provider sqlserver --connection "..." --table "dbo.TableName"
dotnet run --project src/DatabaseMigrationTool -- config --create-sample ./sample.json
...
- `src/DatabaseMigrationTool/` - Main WPF application with CLI support
  ...
  - `Helpers/` - Connection string builders and helper utilities
  ...
  - `sample_config.json` - Configuration file template
...
- `DatabaseExporter` - Schema and data export with overwrite detection
...
- `MetadataManager` - Granular export metadata (JSON manifests + .meta files)
- `ValidationService` - Data integrity and schema validation
- `ConfigurationValidator` - Configuration file validation
- `ConnectionProfileManager` - Connection profile management
- `DirectImporter` - Direct data import bypassing batch processing
- `DirectTransferUtility` - Direct table-to-table transfer
- `EmergencyImporter` - Emergency import from corrupted/failed exports
- `FirebirdContextConnector` & `FirebirdProcessConnector` - Firebird-specific connection management
- `OperationRecoveryService` - Recovery from failed operations
- `OperationStateManager` - Operation state tracking
- `StreamingDataReader` - Memory-efficient data streaming
- `UserSettingsService` - User preferences management

### UI Layer (WPF)
- `MainWindow` - Primary tabbed interface
- `ConnectionStringControl` - Unified connection configuration
- `TableSelectionWindow` - Multi-select table browser with search
- `SchemaViewWindow` - Database schema visualization
- Specialized dialogs for overwrite handling and recovery

### Export/Import Format
- Granular metadata system with BZip2-compressed MessagePack schemas
- Batch processing for large tables with configurable sizes
- Foreign key dependency tracking for proper import ordering
- Cross-database type conversion and migration support
...
**CRITICAL: These rules must be strictly followed for all Firebird-related development:**

1. **Firebird DLL Storage**: Store DLLs in project folder structure:
   - `FirebirdDlls\v25` for Firebird 2.5 files
   - `FirebirdDlls\v50` for Firebird 5.0 files

2. **Build Time Deployment**:
   - FB 5.0 DLLs copied to base output folder
   - FB 2.5 files copied to v25 subfolder under output

3. **Embedded Mode Only**: All connections use embedded mode (ServerType=1)

4. **engine13.dll Placement**: Must be in TWO locations:
   - Base output folder: `engine13.dll`
   - Plugins subfolder: `plugins\engine13.dll`

5. **NO plugins.conf**: File must NOT be present (causes configuration conflicts)
...
The tool supports JSON configuration files for complex migration scenarios:
```bash
# Create sample configuration
dotnet run --project src/DatabaseMigrationTool -- config --create-sample ./config.json

# Validate configuration
dotnet run --project src/DatabaseMigrationTool -- config --validate ./config.json

# Use configuration file (overrides command-line parameters)
dotnet run --project src/DatabaseMigrationTool -- export --config ./config.json
```

## Key Dependencies

- **CommandLineParser** - CLI argument parsing and validation
- **Serilog** - Structured logging with multiple sinks
- **MessagePack** - High-performance binary serialization
- **SharpCompress** - BZip2 compression for metadata
- **FirebirdSql.Data.FirebirdClient** - Firebird database connectivity
...
5. **Debugging**: Use diagnostic commands (`diagnose`, `validate-ei`) for troubleshooting
